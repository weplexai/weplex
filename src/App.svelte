<script lang="ts">
  ...
  const VALID_PATTERNS = [
    'dots', 'polka', 'grid', 'lines', 'crosshatch',
    'vertical', 'horizontal', 'plus', 'isometric',
  ] as const;
  let spacePattern = $derived(
    VALID_PATTERNS.includes(spaceStore.activeSpace.bgPattern as never)
      ? spaceStore.activeSpace.bgPattern!
      : 'none',
  );
...
<div
  ...
  style={spaceBgColor
    ...
    : ''}
>
  {#if spacePattern !== 'none'}
    <div class="app-pattern" data-pattern={spacePattern}></div>
  {/if}
  {#if spaceGrain > 0}
...
<style>
  ...
  /* Pattern overlay — rendered between bgColor and grain.
     Uses currentColor so single SVG works for any space tint. */
  .app-pattern {
    position: absolute;
    inset: 0;
    z-index: 0;
  ...
  .app-pattern[data-pattern='dots'] {
    background-image: radial-gradient(currentColor 1px, transparent 1.2px);
    background-size: 18px 18px;
  }

  .app-pattern[data-pattern='grid'] {
    background-image:
  ...
  .app-pattern[data-pattern='lines'] {
  ...
  .app-pattern[data-pattern='crosshatch'] {
    background-image:
      repeating-linear-gradient(45deg, currentColor 0, currentColor 1px, transparent 1px, transparent 14px),
      repeating-linear-gradient(-45deg, currentColor 0, currentColor 1px, transparent 1px, transparent 14px);
  }

  .app-pattern[data-pattern='vertical'] {
    background-image: repeating-linear-gradient(
      90deg, currentColor 0, currentColor 1px, transparent 1px, transparent 16px
    );
  }

  .app-pattern[data-pattern='horizontal'] {
    background-image: repeating-linear-gradient(
      0deg, currentColor 0, currentColor 1px, transparent 1px, transparent 16px
    );
  }

  .app-pattern[data-pattern='polka'] {
    background-image:
  ...
  .app-pattern[data-pattern='isometric'] {
    background-image:
      repeating-linear-gradient(60deg, transparent 0, transparent 28px, currentColor 28px, currentColor 29px),
      repeating-linear-gradient(-60deg, transparent 0, transparent 28px, currentColor 28px, currentColor 29px);
  }

  /* Plus uses mask-image so the fill inherits currentColor (low-opacity white)
     consistent with CSS-gradient patterns; background-image SVG would ignore
     currentColor because data URLs render in their own context. */
  .app-pattern[data-pattern='plus'] {
    -webkit-mask-image: url("data:image/svg+xml;utf8,%3Csvg xmlns='http://www.w3.org/2000/svg' width='20' height='20' viewBox='0 0 20 20'%3E%3Cg fill='black'%3E%3Crect x='9' y='6' width='2' height='8' rx='1'/%3E%3Crect x='6' y='9' width='8' height='2' rx='1'/%3E%3C/g%3E%3C/svg%3E");
    mask-image: url("data:image/svg+xml;utf8,%3Csvg xmlns='http://www.w3.org/2000/svg' width='20' height='20' viewBox='0 0 20 20'%3E%3Cg fill='black'%3E%3Crect x='9' y='6' width='2' height='8' rx='1'/%3E%3Crect x='6' y='9' width='8' height='2' rx='1'/%3E%3C/g%3E%3C/svg%3E");
