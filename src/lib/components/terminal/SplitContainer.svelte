<script lang="ts">
  ...
  $effect(() => {
    ...
    function tryMount() {
      ...
      // Force xterm to redraw — Canvas doesn't refresh on reparent when slot
      // size is unchanged. rAF lets layout settle before fit() reads dimensions.
      requestAnimationFrame(() => terminalRegistry.refresh(sessionId));
    }
