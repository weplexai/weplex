<script lang="ts">
  ...
  let currentWorkflow = $derived(session.workflowId || '');
  ...
  function changeWorkflow(id: string) {
    sessionStore.update(session.id, { workflowId: id || undefined });
    const fresh = sessionStore.sessions.find((s) => s.id === session.id);
    if (fresh) contextInjectionStore.inject(fresh);
  }
...
<section class="section">
  ...
  <Select
    ...
    onchange={(v) => changeWorkflow(v)}
  />
  {#if session.agentType && session.agentType !== 'claude' && currentWorkflow}
    <p class="workflow-hint">Injection works with Claude only</p>
  {/if}
</section>
