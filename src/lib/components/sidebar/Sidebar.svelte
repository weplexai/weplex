<script lang="ts">
  ...
  import { findDropTarget, executeDrop } from '../../utils/sidebarDragDrop';
  ...
  function handlePointerMove(e: PointerEvent) {
    ...
    const target = scrollEl ? findDropTarget(e.clientX, e.clientY, scrollEl, spaceId) : null;
  ...
  // One physical swipe = one space switch. After switching, all wheel events
  // are eaten for 300ms (matching animation), then next swipe is allowed.
  ...
  function handleSwipeWheel(e: WheelEvent) {
    ...
    if (swipeOffset < -w * 0.4 && canNext) {
      ...
      swipeOffset = 0;
      setTimeout(() => { snapping = false; switched = false; }, 300);
      return;
    }
      ...
      >
        ...
        {#each visibleSpaces as space (space.id)}
          ...
          {@const splitIds = splitStore.hasSplits(space.id)
            ...
            : []}
          <div class="slider-slide">
            <!-- Space header (Arc-style: badge + name) -->
            <div class="space-header">
              <SpaceBadge
                letter={(space.name?.[0] ?? '?').toUpperCase()}
                color={space.color}
              />
              <span class="space-header-name" title={space.name}>{space.name}</span>
            </div>
