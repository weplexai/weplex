...
use weplex_mcp_contract::{
    METHOD_INITIALIZE, METHOD_NOTIFICATIONS_INITIALIZED, METHOD_TOOLS_CALL, METHOD_TOOLS_LIST,
};
...
fn handle_request(
    ...
    match request.method.as_str() {
        m if m == METHOD_INITIALIZE => Some(JsonRpcResponse::success(
            request.id.clone(),
            mcp_protocol::initialize_result(),
        )),

        // Notification — no response expected
        m if m == METHOD_NOTIFICATIONS_INITIALIZED => None,

        m if m == METHOD_TOOLS_LIST => {
            let tools = tools::list_tools(global_socket_path);
            Some(JsonRpcResponse::success(request.id.clone(), tools))
        }

        m if m == METHOD_TOOLS_CALL => {
